...
vw_ml_loan AS 
SELECT * FROM fact.loans;

CREATE OR REPLACE PROCEDURE etl.etl_clean_to_fact_loans()
LANGUAGE plpgsql
AS $$
DECLARE
    v_success INT := 0;
...
BEGIN
    ...
    BEGIN
        WITH raw_loans_deduplicated AS (
            SELECT DISTINCT ON(
        ...
        SELECT 
            ...
            CASE 
                ...
                WHEN NOT (CAST(credit_score AS NUMERIC) BETWEEN 300 AND 850) THEN 'Invalid credit_score range: ' || credit_score
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            INSERT INTO raw.etl_log(date_time, status, success_count, reject_count, error_message)
